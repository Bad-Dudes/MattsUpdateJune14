...
public class Enemy
{
  ...
  public void enemyReader() {
    ...
    for(int i = 0; i < xL; i++){
      for(int z = 0; z < yL; z++){
        if(enemyLayout[i][z] == 's'){
          sList[skeletonNumber2] = new Skeleton(32*(i-1),32*(z+1),0,0,game,level,enemytxt);
          skeletonNumber2++;
          
        }
        if(enemyLayout[i][z] == 'u'){
          gList[ghostNumber2] = new Ghost(32*(i-1),32*(z+1),0,0,game,level,enemytxt);
          ghostNumber2++;
        }
        if(enemyLayout[i][z] == 'h'){
          ghList[ghoulNumber2] = new Ghoul(32*(i-1),32*(z+1),0,0,game,level,enemytxt);
          ghoulNumber2++;
        }
        y = y + height;
      }
